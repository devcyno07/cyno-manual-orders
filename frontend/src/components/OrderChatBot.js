...
const BOT_QUESTIONS = {
  en: {
    ...
    review:        "📋 Here's your **order summary**. Everything look good?",
...
export default function OrderChatBot() {
  ...
  const advance = useCallback((nextIdx, data = {}) => {
    ...
    if (q) botSay(q, {}, 500);
  }, [formData, bankDetails, cartItems, questions, lang, botSay]);
  ...
  }, [input, currentStep, stepIdx, advance, addMessage, lang, botSay]);
  ...
  }, [currentStep, stepIdx, advance, addMessage]);
  ...
  }, [lang, botSay, stepIdx, advance, addMessage]);
  ...
  }, [cartItems, lang, botSay, stepIdx, advance, addMessage]);
            ...
            <div key={msg.id} className={`${styles.msgRow} ${msg.role === 'user' ? styles.msgUser : styles.msgBot}`}>
              ...
              <div className={styles.bubble}>
                {msg.extra?.type === 'review' ? (
                  <ReviewCard formData={msg.extra.formData} cartItems={msg.extra.cartItems} lang={lang} />
          ...
          {showTextInput && (
            <div className={styles.textInputWrap}>
              <input
                ...
                value={input}
                onChange={e => setInput(e.target.value)}
                placeholder={currentStep.optional
...
// ── Review Card ────────────────────────────────────────────────────────────
function ReviewCard({ formData, cartItems, lang }) {
  const total = cartItems?.reduce((s, i) => s + i.price * i.quantity, 0) || 0;
  const rows = lang === 'zh'
    ? [
        ['姓名', formData.customerName],
        ['电子邮件', formData.customerEmail],
        ['联系电话', formData.contactNumber],
        ['汇款人', formData.remitterName],
        ['收货人', formData.addressFullName],
        ['性别', formData.sex],
        ['年龄', formData.age],
        ['地址', [formData.addressLine1, formData.addressLine2].filter(Boolean).join(', ')],
        ['城市', formData.city],
        ['省份', formData.state],
        ['邮编', formData.postalCode],
        ['国家', formData.country],
        ['电话', formData.addressPhone],
      ]
    : [
        ['Name', formData.customerName],
        ['Email', formData.customerEmail],
        ['Contact', formData.contactNumber],
        ['Remitter', formData.remitterName],
        ['Consignee', formData.addressFullName],
        ['Sex', formData.sex],
        ['Age', formData.age],
      ...
      <div style={{ borderTop: '1px solid #f0fdf4', paddingTop: 8 }}>
        {rows.filter(([, v]) => v).map(([k, v]) => (
          <div key={k} style={{ display: 'flex', gap: 8, padding: '2px 0' }}>
            <span style={{ color: '#5a8090', minWidth: 70 }}>{k}</span>
